...
export const RecuperarContrasena = () => {
  ...
  useEffect(() => {
    // Verificar si el token corresponde a algún usuario en localStorage
    setTimeout(() => {
      const usuarios = JSON.parse(localStorage.getItem('usuarios_db') || '[]');
      const usuarioConToken = usuarios.find((u) => u.tokenRecuperacion === token);
      setTokenValido(!!usuarioConToken);
      setLoading(false);
    }, 500);
  }, [token]);
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      const usuarios = JSON.parse(localStorage.getItem('usuarios_db') || '[]');
      const index = usuarios.findIndex((u) => u.tokenRecuperacion === token);
      if (index >= 0) {
        usuarios[index].contraseña = nuevaContrasena;
        delete usuarios[index].tokenRecuperacion;
